...
import { Clock, Play, Eye, Heart, X } from 'lucide-react';
import { formatDate, formatViews, formatDuration } from '../lib/utils';

export const WatchLaterPage: React.FC = () => {
  const navigate = useNavigate();

  const { data: watchLater, isLoading, refetch } = useQuery({
    queryKey: ['watch-later'],
    queryFn: () => userService.getWatchLater(),
  });
        ...
        {!isLoading && watchLater && (
          <>
            {watchLater.data.length === 0 ? (
              ...
              >
                <BackgroundGradient className="p-12 max-w-2xl mx-auto">
                  ...
                  <h2 className="text-2xl font-bold mb-4">No Videos in Queue</h2>
                  <p className="text-gray-400 mb-6">
                    Add videos to your watch later queue to view them here!
                  </p>
                </BackgroundGradient>
              </motion.div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                {watchLater.data.map((video: any, index: number) => (
                  <motion.div
                    key={video._id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    ...
                    <BackgroundGradient className="p-0 h-full overflow-hidden">
                      <div className="relative group h-full flex flex-col">
                        ...
                        <div className="relative aspect-video bg-slate-800 overflow-hidden flex-shrink-0">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/50">
                            <motion.div 
                              ...
                              className="w-12 h-12 sm:w-16 sm:h-16 bg-blue-600 rounded-full flex items-center justify-center shadow-lg shadow-blue-500/50"
                            >
                        ...
                        <div className="p-3 sm:p-4 flex-grow flex flex-col">
                          <h3 className="text-base sm:text-lg font-semibold mb-2 line-clamp-2 group-hover:text-blue-400 transition-colors">
                            {video.title}
                          </h3>
            ...
            {watchLater.data.length > 0 && (
              ...
              >
                <p>{watchLater.data.length} video{watchLater.data.length !== 1 ? 's' : ''} in queue</p>
              </motion.div>
            )}
          </>
        )}
